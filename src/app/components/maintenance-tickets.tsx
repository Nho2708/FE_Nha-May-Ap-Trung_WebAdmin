...
export function MaintenanceTickets() {
  ...
  const loadIncubatorSerials = async () => {
    try {
      ...
      setIncubatorSerialMap(map);
    } catch {
      // giữ map rỗng nếu lỗi
    }
  };
  ...
  const handleStatusUpdate = async (status: MaintenanceTicketStatus) => {
    ...
    if (needsSummary && !progressNote.trim()) {
      setError("Vui lòng nhập tóm tắt kết quả xử lý vào ô bên trên trước khi đánh dấu.");
      return;
    }
          ...
          {detailLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <DetailRow label="Máy ấp">{incubatorSerialMap.get(selectedTicket.incubatorId) ?? selectedTicket.incubatorId}</DetailRow>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Ví dụ: Đã liên hệ khách hàng, đang chờ linh kiện thay thế... (bắt buộc khi đánh dấu Đã xử lý)"
