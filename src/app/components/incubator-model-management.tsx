...
function ImageUploader({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadError(null);
    try {
      const url = await incubatorModelService.uploadImage(file);
      onUrlChange(url);
    } catch (err) {
  ...
  return (
    <div className="space-y-2">
      {currentUrl && (
        <div className="relative w-full h-44 rounded-lg overflow-hidden border border-slate-200 bg-slate-50">
          <img src={currentUrl} alt="Ảnh dòng máy" className="w-full h-full object-contain" />
          <button
            type="button"
      ...
      <label
        ...
        className={`flex items-center justify-center gap-2 w-full px-4 py-2.5 border-2 border-dashed border-slate-300 rounded-lg text-sm transition-colors
          ${uploading ? "opacity-60 pointer-events-none text-slate-400" : "cursor-pointer hover:border-blue-400 hover:bg-blue-50 text-slate-500 hover:text-blue-600"}`}
      >
        {uploading ? (
          <><Loader2 size={15} className="animate-spin" /> Đang upload lên Cloudinary...</>
        ) : (
          <><ImageIcon size={15} /> {currentUrl ? "Thay ảnh khác" : "Chọn ảnh (JPEG, PNG, WEBP)"}</>
        )}
      </label>
          ...
          {detailLoading ? (
            ...
            <div className="space-y-4">
              ...
              {selectedModel.imageUrl ? (
                ...
                <div className="w-full h-32 rounded-xl border border-dashed border-slate-200 bg-slate-50 flex flex-col items-center justify-center gap-1">
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <OptionalLabel>Hình ảnh dòng máy</OptionalLabel>
          <ImageUploader currentUrl={imageUrl} onUrlChange={setImageUrl} />
        </div>
...
function EditModelPanel({
  ...
  useEffect(() => {
    if (model) {
      ...
      setConfigs([]);
      setImageUrl(model.imageUrl ?? null);
    }
  }, [model]);
